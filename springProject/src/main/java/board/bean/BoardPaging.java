...
public class BoardPaging {
	...
	public void makePagingHTML() {
		...
		if (startPage > pageBlock) {
			pagingHTML
					.append("[<a id='paging' href='/springProject/board/boardList?pg=" + (startPage - 1) + "'>이전</a>]");
		}

		for (int i = startPage; i <= endPage; i++) {
			if (i == currentPage) {
				pagingHTML.append(
						"[<a id='currentPaging' href='/springProject/board/boardList?pg=" + i + "'>" + i + "</a>]");
			} else {
	...
	public void makeSearchPagingHTML() {
		...
		for (int i = startPage; i <= endPage; i++) {
			if (i == currentPage) {
				pagingHTML.append("[<span id='currentPaging' onclick='boardList(" + i + ")'>" + i + "</span>]");
			} else {
